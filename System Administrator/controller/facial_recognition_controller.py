...
class FaceDetector:
    ...
    def is_valid_face(face_box: np.ndarray, frame_width: int, frame_height: int) -> bool:
        ...
        # Check aspect ratio: faces are typically 0.7 to 1.4 (height/width)
        # Hands and arms usually have very different ratios
        aspect_ratio = h / max(w, 1)
        if aspect_ratio < 0.7 or aspect_ratio > 1.4:
            return False

        # Check relative size: face should be reasonable size of frame
        # Too small = noise, too large = probably not a face
        face_area = w * h
        frame_area = frame_width * frame_height
        relative_size = face_area / frame_area

        if relative_size < 0.02 or relative_size > 0.7:  # 2% to 70% of frame
            return False

        # Check minimum absolute size (at least 60x60 pixels)
        if w < 60 or h < 60:
            return False
...
class FaceDatabase:
    ...
    def find_match(self, feature: np.ndarray, threshold: float = 0.30, min_confidence_gap: float = 0.020) -> Optional[Tuple[str, str, float]]:
        """
        Find the best matching student for a given face feature.

        Args:
            feature: Face feature vector to match
            threshold: Minimum similarity threshold (0.30 - balanced for accuracy)
            min_confidence_gap: Minimum gap between best and second-best match (0.020 - ensures clear distinction)
...
class FaceTracker:
    ...
    def __init__(self, max_age: int = 10):
        ...
        """
        self.tracks = {}  # track_id -> {student_id, last_seen, position, confirmation_count}
        self.next_track_id = 0
        self.max_age = max_age
        self.confirmation_threshold = 1  # Require only 1 detection for immediate recognition
    ...
    def update(self, detections: List[Tuple[Tuple[int, int, int, int], str, str, float]]) -> List[Tuple[str, str, float, bool]]:
        ...
        for bbox, student_id, name, confidence in detections:
            ...
            if best_track_id is not None:
                ...
                self.tracks[best_track_id]['position'] = center
                self.tracks[best_track_id]['confirmation_count'] = min(
                    self.tracks[best_track_id]['confirmation_count'] + 1,
                    self.confirmation_threshold + 5
                )
                self.tracks[best_track_id]['confidence'] = max(
                    self.tracks[best_track_id]['confidence'],
                    confidence
                )
            else:
                # Create new track
    ...
    try:
        ...
        db_utils.execute(
            """INSERT INTO attendance
               ...
               VALUES (%s, %s, %s, 'present', %s, FALSE)""",
            (student_data['id'], module_id, timetable_id, confidence)
        )
